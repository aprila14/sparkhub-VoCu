...
static EPacketHandlingResult handleResponse_lightSetLevel(
        BleController* pController,
        const uint8_t* payload,
        uint16_t payloadLen)
{
    UNUSED(pController);
    LOG_DEBUG("Handling response for 'Light Set Level command...");

    if (payloadLen != sizeof(prot::light_set_level::TRes))
    {
        LOG_WARNING("Invalid 'Light Set Level' response. Payload legnth: %d", payloadLen);
        return EPacketHandlingResult::SEND_NACK;
    }

    prot::light_set_level::TRes* pRes = (prot::light_set_level::TRes*) payload;

    LOG_INFO("Light level set successfully");
        ...
        uint16_t payloadLen)
{
    switch (packetType)
    {
        ...
        case prot::EPacketType::RES_LIGHT_SET_LEVEL:
            return handleResponse_lightSetLevel(pController, payload, payloadLen);
