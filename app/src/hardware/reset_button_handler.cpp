...
bool ResetButtonHandler::init() const
{
    ...
    gpioConfig.pull_up_en = GPIO_PULLUP_DISABLE; // hardware pull down needs to be used, button used also during booting
                                                 // for factory firmware reset
    gpioConfig.pull_down_en = GPIO_PULLDOWN_DISABLE; // hardware pull down needs to be used, button used also during
                                                     // booting for factory firmware reset
...
void ResetButtonHandler::runTask()
{
    ...
    BaseType_t xReturned = xTaskCreate(_run, LOG_TAG, DEFAULT_STACK_SIZE, this, DEFAULT_TASK_PRIORITY, &m_taskHandle);
    if (xReturned != pdPASS)
...
void ResetButtonHandler::perform()
{
    ...
    if (state == EResetButtonState::PRESSED)
    {
        ...
        if ((m_buttonPressedDurationMs > DEBOUNCING_DELAY) &&
            (m_buttonPressedDurationMs < MAX_TIME_TO_DETECT_RESET_MS) && !m_softwareRestartReady)
        {
        ...
        if ((m_buttonPressedDurationMs > MIN_TIME_TO_DETECT_CONFIGURATION_RESET_MS) &&
            (m_buttonPressedDurationMs < MAX_TIME_TO_DETECT_CONFIGURATION_RESET_MS) && !m_configurationResetReady)
        {
