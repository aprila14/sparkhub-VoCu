...
cJSON* dataJsonToRpcCommandJson(cJSON* pDataJson, EMsgCode msgCode, uint32_t msgCounter)
{
    ...
    cJSON* pMethodJson = cJSON_CreateString("rpcCommand");

    if (!(cJSON_AddItemToObject(pRpcCommandJson, "method", pMethodJson)))
    {
        LOG_INFO("Cannot add methodJson to rpcCommandJson");
        cJSON_Delete(pParamsJson);
