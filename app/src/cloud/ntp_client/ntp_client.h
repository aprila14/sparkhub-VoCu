...
public:
    ...
    /**
     * @brief getCurrentUtcTimestampMs. Make sure that the time is syncronized first (e.g check getNtpTimeOffset()
     * value)
     * @return
     */
    ...
    /**
     * @brief Called from a global callback from lwip (sntp_sync_time) - therefore we use the same naming as in LWIP
     * here
