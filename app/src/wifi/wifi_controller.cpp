...
:
m_taskHandle(),
m_connectionErrorReason(EConnectToSsidErrorReason::I2A_WIFI_REASON_OTHER),
...
m_lastConnectionErrorReason(EConnectToSsidErrorReason::I2A_WIFI_REASON_NO_ERROR),
m_wifiScanRecords{},
m_scanErrorCode(0)
{
...
bool WiFiController::wifiConfigModeInit(TWiFiCredentials& credentials)
{
    ...
    if (credentials.authorizationMode == i2a_wifi_auth_mode_t::I2A_WIFI_AUTH_NOT_SET)
    {
        if (passwordLength > 0)
        {
            LOG_INFO("I2A_WIFI_AUTH_NOT_SET - and password set - use authorization WIFI_AUTH_WPA2_PSK");
            wifiConfig.sta.threshold.authmode = WIFI_AUTH_WPA2_PSK;
        }
        else
        {
            LOG_INFO("I2A_WIFI_AUTH_NOT_SET - and password empty - use authorization WIFI_AUTH_OPEN");
    ...
    wifiConfig.sta.pmf_cfg.required = (credentials.authorizationMode == i2a_wifi_auth_mode_t::I2A_WIFI_AUTH_WPA3_PSK ||
                                        credentials.authorizationMode ==  i2a_wifi_auth_mode_t::I2A_WIFI_AUTH_WPA2_WPA3_PSK);
...
void WiFiController::wifiEventHandler(void* eventHandlerArg, esp_event_base_t eventBase, int32_t eventId, void* eventData)
{
    ...
    else if (eventBase == WIFI_EVENT && eventId == WIFI_EVENT_STA_BEACON_TIMEOUT)
    {
        ...
        event.reason = EConnectToSsidErrorReason::I2A_WIFI_REASON_OTHER;
        xQueueSend(m_eventsQueue, &event, portMAX_DELAY);
    }
...
EConnectToSsidErrorReason WiFiController::parseWifiDisconnectReason(uint8_t reason)
{
    ...
    switch (reason)
    {
        case WIFI_REASON_UNSPECIFIED:
            ...
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_UNSPECIFIED;
            break;
        case WIFI_REASON_AUTH_EXPIRE:
            LOG_INFO("Connection error, reason: AUTH_EXPIRE");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_AUTH_EXPIRE;
            break;
        case WIFI_REASON_AUTH_LEAVE:
            LOG_INFO("Connection error, reason: AUTH_LEAVE");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_AUTH_LEAVE;
            break;
        case WIFI_REASON_ASSOC_EXPIRE:
            LOG_INFO("Connection error, reason: ASSOC_EXPIRE");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_ASSOC_EXPIRE;
            break;
        case WIFI_REASON_ASSOC_TOOMANY:
            LOG_INFO("Connection error, reason: ASSOC_TOOMANY");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_ASSOC_TOOMANY;
            break;
        case WIFI_REASON_ASSOC_LEAVE:
            LOG_INFO("Station disconnected");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_ASSOC_LEAVE;
            break;
        case WIFI_REASON_MIC_FAILURE:
            LOG_INFO("Connection error, reason: MIC_FAILURE (Message integrity code failure)");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_MIC_FAILURE;
            break;
        case WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT:
            LOG_INFO("Reason: 4WAY_HANDSHAKE_TIMEOUT, probably wrong password");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_4WAY_HANDSHAKE_TIMEOUT;
            break;
        case WIFI_REASON_NO_AP_FOUND:
            LOG_INFO("Reason: NO_AP_FOUND (No Access Point found), wrong ssid/authorization method");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_NO_AP_FOUND;
            break;
        case WIFI_REASON_AUTH_FAIL:
            LOG_INFO("Reason: AUTH_FAIL (Other authenthication issue)");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_AUTH_FAIL;
            break;
        case WIFI_REASON_ASSOC_FAIL:
            LOG_INFO("Reason: ASSOC_FAIL (Other association failure)");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_ASSOC_FAIL;
            break;
        case WIFI_REASON_CONNECTION_FAIL:
            LOG_INFO("Reason: CONNECTION_FAIL (Connection failed with Access Point)");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_CONNECTION_FAIL;
            break;
        case WIFI_REASON_BEACON_TIMEOUT:
            LOG_INFO("Reason: BEACON_TIMEOUT, connection lost with Access Point");
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_BEACON_TIMEOUT;
            break;
        default:
            LOG_INFO("Another reason for disconnection, check error code: %d", reason);
            outputReason = EConnectToSsidErrorReason::I2A_WIFI_REASON_OTHER;
    }
    return outputReason;
}
...
bool WiFiController::initiateDisconnection()
{
    ...
    if (!(xEventGroupWaitBits(m_eventGroup,
                            ...
                            pdFALSE,
                            pdFALSE,
                            MS_TO_TICKS(DISCONNECT_TIMEOUT_MS)) & EVENT_BIT_WIFI_STOPPED))
    {
        return false;
    }
    else
...
bool WiFiController::disconnectIfNeeded()
{
    if (m_connectionStatus)
    {
        ...
        if (!(xEventGroupWaitBits(m_eventGroup,
                                ...
                                pdFALSE,
                                pdFALSE,
                                MS_TO_TICKS(SHORT_TIMEOUT_MS)) & EVENT_BIT_WIFI_STOPPED))
        {
        ...
        bool shouldReconnect)
{
    ...
    result.status = EConnectToSsidErrorReason::I2A_WIFI_REASON_OTHER;
    ...
    if (!(xEventGroupWaitBits(m_eventGroup,
                             EVENT_BIT_WIFI_CREDENTIALS_READY,
                             pdTRUE,
                             pdFALSE, SHORT_TIMEOUT_MS) & EVENT_BIT_WIFI_CREDENTIALS_READY))
    {
    ...
    if (!(xEventGroupWaitBits(m_eventGroup,
                            ...
                            pdTRUE,
                            pdFALSE,
                            MS_TO_TICKS(command.timeoutMs)) & EVENT_BIT_WIFI_IP_ASSIGNED))
    {
        result.succeded = false;
        m_shouldReconnect = false;

        if (m_lastConnectionErrorReason != EConnectToSsidErrorReason::I2A_WIFI_REASON_NO_ERROR)
        {
            result.status = m_lastConnectionErrorReason;
        }
        else
        {
            result.status = EConnectToSsidErrorReason::I2A_WIFI_REASON_TIMEOUT;
        }
    ...
    else
    {
        ...
        result.status = EConnectToSsidErrorReason::I2A_WIFI_REASON_NO_ERROR;
    }
...
bool WiFiController::scanAvailableAccessPoints(prot::wifi_scan_ap::TRes& result)
{
    ...
    if (!(xEventGroupWaitBits(m_eventGroup,
                              ...
                              pdTRUE,
                              pdFALSE,
                              MS_TO_TICKS(SCAN_TIMEOUT_MS)) & EVENT_BIT_WIFI_SCAN_RESULTS_READY))
    {
...
bool WiFiController::waitUntilConnectedToWifi() const
{
    ...
    if (!(xEventGroupWaitBits(m_eventGroup,
                            ...
                            pdFALSE,
                            pdFALSE,
                            portMAX_DELAY) & EVENT_BIT_WIFI_CONNECTED))
    {
...
prot::wifi_scan_ap::TScanEntry WiFiController::convertRecordToScanEntry(wifi_ap_record_t& record)
{
    ...
    result.secondaryChannel = static_cast<i2a_wifi_second_chan_t>(record.second);
    result.rssi = record.rssi;
    result.authmode = static_cast<i2a_wifi_auth_mode_t>(record.authmode);
