...
bool WiFiController::init()
{
    ...
    xEventGroupSetBits(m_eventGroup, EVENT_BIT_WIFI_STOPPED | EVENT_BIT_WIFI_DISCONNECTED);
    xEventGroupClearBits(
        m_eventGroup,
        EVENT_BIT_WIFI_CREDENTIALS_READY | EVENT_BIT_WIFI_CONNECTED | EVENT_BIT_WIFI_STARTED |
            EVENT_BIT_WIFI_PUBLIC_INTERFACE_HANDLING | EVENT_BIT_WIFI_IP_ASSIGNED);
...
bool WiFiController::wifiConfigModeInit(TWiFiCredentials& credentials)
{
    ...
    wifiConfig.sta.pmf_cfg.required =
        (credentials.authorizationMode == device_wifi_auth_mode_t::DEVICE_WIFI_AUTH_WPA3_PSK ||
...
{
    if (esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, _wifiEventHandler, this, NULL) != ESP_OK)
    {
        LOG_ERROR("Error while registering wifiEventHandler for WIFI_EVENT");
        return false;
    }

    if (esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, _wifiEventHandler, this, NULL) != ESP_OK)
    {
...
void WiFiController::wifiEventHandler(
    ...
    void*            eventData)
{
    ...
    {
        wifi_event_sta_disconnected_t* disconnectionEvent =
            reinterpret_cast<wifi_event_sta_disconnected_t*>(eventData); // NOLINT
...
void WiFiController::processWifiEvent(TWifiEvent& event)
{
    ...
    switch (event.eventType)
    {
        ...
        case EWifiEvent::WIFI__EVENT_DISCONNECTED:
        {
            ...
            if (getConnectionStatus())
            {
                app::pAppController->addEvent(
                    app::EEventType::WIFI_CONTROLLER__CONNECTION_LOST, app::EEventExecutionType::ASYNCHRONOUS, nullptr);
            }
        ...
        case EWifiEvent::WIFI__EVENT_IP_ASSIGNED:
        {
            ...
            app::pAppController->addEvent(
                app::EEventType::WIFI_CONTROLLER__CONNECTION_ESTABLISHED,
...
void WiFiController::tryToReconnect()
{
    if (m_shouldReconnect && !m_disconnectionInitiated)
    {
        if (!(xEventGroupWaitBits(
                  ...
                  pdTRUE,
                  pdFALSE,
                  MS_TO_TICKS(RECONNECTION_INTERVAL_MS)) &
              EVENT_BIT_WIFI_PUBLIC_INTERFACE_HANDLING))
        {
...
bool WiFiController::initiateDisconnection()
{
    ...
    if (!(xEventGroupWaitBits(
              m_eventGroup, EVENT_BIT_WIFI_STOPPED, pdFALSE, pdFALSE, MS_TO_TICKS(DISCONNECT_TIMEOUT_MS)) &
...
bool WiFiController::disconnectIfNeeded()
{
    if (m_connectionStatus)
    {
        ...
        if (!(xEventGroupWaitBits(
                  m_eventGroup, EVENT_BIT_WIFI_STOPPED, pdFALSE, pdFALSE, MS_TO_TICKS(SHORT_TIMEOUT_MS)) &
              EVENT_BIT_WIFI_STOPPED))
        {
    ...
    bool                                shouldReconnect)
{
    ...
    xEventGroupClearBits(m_eventGroup, EVENT_BIT_WIFI_CREDENTIALS_READY | EVENT_BIT_WIFI_IP_ASSIGNED);
    ...
    if (!(xEventGroupWaitBits(m_eventGroup, EVENT_BIT_WIFI_CREDENTIALS_READY, pdTRUE, pdFALSE, SHORT_TIMEOUT_MS) &
    ...
    if (!(xEventGroupWaitBits(
              m_eventGroup, EVENT_BIT_WIFI_IP_ASSIGNED, pdTRUE, pdFALSE, MS_TO_TICKS(command.timeoutMs)) &
...
bool WiFiController::scanAvailableAccessPoints(prot::wifi_scan_ap::TRes& result)
{
    ...
    if (!(xEventGroupWaitBits(
              m_eventGroup, EVENT_BIT_WIFI_SCAN_RESULTS_READY, pdTRUE, pdFALSE, MS_TO_TICKS(SCAN_TIMEOUT_MS)) &
...
bool WiFiController::waitUntilConnectedToWifi() const
{
    ...
    if (!(xEventGroupWaitBits(m_eventGroup, EVENT_BIT_WIFI_CONNECTED, pdFALSE, pdFALSE, portMAX_DELAY) &
